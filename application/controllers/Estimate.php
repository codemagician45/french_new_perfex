...
class Estimate extends ClientsController
{
    public function index($id, $hash)
    {
        ...
        if ($this->input->post('estimatepdf')) {
            ...
            $test_pdf= $pdf->Output($filename,'S');
            file_put_contents('filename.pdf', $test_pdf);
    ...
    public function universign_return($id,$hash)
    {
        ...
        if(!$r->faultCode()) {
            ...
            ob_clean();
            flush();
            file_put_contents('uploads/pdf/'.$name ,$doc['content']);
            $data['signed_pdf'] = 'uploads/pdf/'.$name;
    ...
    {
        // print_r(APP_BASE_URL.'uploads/pdf'); exit();
        $doc = $_SESSION['signed_documents'][0];
        $name = $doc['name'];
        //Retrieve the document
        header("Pragma: no-cache");
