...
<div id="wrapper">
					...
					<div class="panel-body">
						<div class="_buttons">
							<a href="#" onclick="new_status(); return false;" class="btn btn-info pull-left display-block">
								<?php echo _l('reminder_new_status'); ?>
							</a>
						</div>
						...
						<table class="table dt-table scroll-responsive" data-order-col="1" data-order-type="asc">
							<thead>
								...
								<th><?php echo _l('reminders_status_table_name'); ?></th>
								...
								<tr>
									...
									<td><a href="#" onclick="edit_status(this,<?php echo $status['id']; ?>);return false;" data-color="<?php echo $status['color']; ?>" data-name="<?php echo $status['name']; ?>" data-order="<?php echo $status['statusorder']; ?>"><?php echo $status['name']; ?></a><br />
										<span class="text-muted">
											<?php echo _l('reminders_table_total',total_rows(db_prefix().'reminders',array('status'=>$status['id']))); ?></span>
										</td>
										<td>
											...
											<a href="<?php echo admin_url('misc/delete_reminder_status/'.$status['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
											<?php } ?>
										</td>
									</tr>
									<?php } ?>
							...
							<?php } else { ?>
							<p class="no-margin"><?php echo _l('reminders_statuses_not_found'); ?></p>
							<?php } ?>
						</div>
					</div>
				</div>
	...
	<?php include_once(APPPATH.'views/admin/reminders/status.php'); ?>
	<?php init_tail(); ?>
</body>
</html>
